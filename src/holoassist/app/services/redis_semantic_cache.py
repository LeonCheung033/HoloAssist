...
class RedisSemanticCache:
    ...
    async def get_cache(self, query: str, embedding: List[float]) -> Optional[str]:
        """查询语义缓存
        ...
        try:
            ...
            if best_match and best_similarity >= self.cache_threshold:
                logger.info(
                    f"Cache hit (similarity: {best_similarity:.3f}) for query: {query[:50]}..."
                )
                return best_match
    ...
    async def set_cache(self, query: str, response: str, embedding: Optional[List[float]] = None):
        """设置语义缓存
        ...
        try:
            ...
            await client.setex(
                embedding_key, self.cache_expire, json.dumps(embedding)
            )
